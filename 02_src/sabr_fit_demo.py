...
f = open(os.path.join(data_path, '300etf_option2208.pkl'),'rb')
...
# date1 = np.unique(option2207['date'])[-2]
# insample = option2207.loc[option2207['date'] < date1, :]

params = {}
param = np.array([0.2, 1, 1, -0.2])
r = 0
def pandas_apply_sabr(temp):
    ...
    param_n = sabr.haganFit_delta(sabr.BS, price, atm_index, param)
    # print(param_n)
    if abs(param_n[2] - param[2]) < 1:
        param = param_n
...
insample = option2207.groupby('time').progress_apply(pandas_apply_sabr).reset_index(drop = True)
insample = insample.drop(columns = 'date')
insample.to_pickle(os.path.join(data_path, '300etf_2208_sabr_v1.pkl'))
...
plt.figure()
for i in range(100, 196):
    y = np.linspace(4, 4.9, 100)
    df = insample.loc[insample['time'] == params1.columns[i],:]
    ...
    plt.plot(y, vol, c = cmap(i/96))

for i in range(200, 210):
    time = params1.columns[i]
    df = insample.loc[insample['time'] == time,:]
